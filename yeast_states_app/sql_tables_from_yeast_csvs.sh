...
docker exec escalation-os-psql psql -h localhost -p 5432 -U escalation -d escalation -c "alter table growth_rate add column experiment_id_long text; update growth_rate set experiment_id_long = 'experiment.transcriptic.' || experiment_id;"
sqlacodegen postgresql+pg8000://escalation_os:escalation_os_pwd@localhost:5432/escalation_os --outfile app_deploy_data/models.py
