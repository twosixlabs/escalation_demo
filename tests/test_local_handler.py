...
def make_local_handler():
    got_data = LocalHandler("tests/test_data/penguins_size_small")
    return got_data


def test_local_handler_init(make_local_handler):
    assert make_local_handler.file_path == "tests/test_data/penguins_size_small/penguins_size_small.csv"
